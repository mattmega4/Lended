...
class EntryViewController: UIViewController {
    ...
    func signUserIn() {
        ...
        Auth.auth().signIn(withEmail: email, password: password, completion: { (user, error) in
            ...
            if error == nil {
                ref.child("users").child((user?.uid)!).child("accounts")
                    .observe(.value, with: { snapshot in
                        if (snapshot.hasChildren()) {
                            
                            if let walletVC = self.storyboard?.instantiateViewController(withIdentifier: "LandingVC") as? LandingPageViewController {
                            ...
                            if let addVC = self.storyboard?.instantiateViewController(withIdentifier: "AddAccountVC") as? AddAccountViewController {
        ...
        if textFieldOne.text == textFieldTwo.text {
            ...
            Auth.auth().createUser(withEmail: newUserEmail!, password: newUserPassword!, completion: { (user, error) in
                ...
                if (error != nil) {
                    ...
                    ref.child("users").child((user?.uid)!).child("accounts").setValue(true)
